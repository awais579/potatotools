...
@section('title', 'Login - PotatoTools')
@section('description', 'Login to PotatoTools with a clean and simple account page built for fast access.')
@section('canonical', route('login'))

@section('content')
    <section class="h-screen overflow-hidden px-4 py-4 sm:px-5 sm:py-5">
        <div class="mx-auto grid h-full w-full max-w-[1140px] overflow-hidden rounded-[2rem] border border-stone-200/70 bg-white/80 shadow-[0_24px_70px_rgba(32,24,16,0.09)] backdrop-blur lg:grid-cols-[minmax(0,0.92fr)_minmax(360px,1.08fr)]">
            <section class="flex items-stretch bg-white p-5 shadow-[18px_0_40px_rgba(32,24,16,0.12)] sm:p-6 lg:p-8">
                <div class="mx-auto flex h-full w-full max-w-md flex-col pt-2 sm:pt-3">
                    ...
                    <div class="flex flex-1 items-center">
                        <div class="w-full">
                            <div class="mb-6">
                                <p class="text-xs font-semibold uppercase tracking-[0.14em] text-stone-500">Account Access</p>
                                <h1 class="mt-2 text-3xl font-semibold text-stone-900 sm:text-4xl">Login to PotatoTools</h1>
                                <p class="mt-2 text-sm leading-relaxed text-stone-600">Use your account details to continue.</p>
                            </div>
                            ...
                            <form method="post" action="{{ route('login.submit') }}" class="{{ session('status') || $errors->any() ? 'mt-6' : '' }} space-y-4">
                                @csrf
                                ...
                                <div class="flex flex-col gap-2 text-sm text-stone-600 sm:flex-row sm:items-center sm:justify-between">
                                    <label class="inline-flex cursor-pointer items-center gap-2">
                                        <input type="checkbox" name="remember" class="h-4 w-4 cursor-pointer rounded border-stone-300 text-primary focus:ring-primary/30">
                                        <span>Remember me</span>
                                    </label>
                                    <span class="font-medium text-stone-500">Forgot password coming soon</span>
                                </div>
                            ...
                            <a href="{{ route('signup') }}" class="pt-btn-secondary mt-3 flex w-full justify-center py-3 text-sm">
